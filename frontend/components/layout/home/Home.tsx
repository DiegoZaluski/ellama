...
import { SiMeta as Llama, SiHuggingface as Huggingface } from 'react-icons/si';
...
function Home() {
  ...
  const BorderCHAT =
    document.documentElement.getAttribute('data-theme') === 'dark' ? 'bg-black' : 'transparent';
  ...
  return (
    ...
    >
      ...
      >
        ...
        ></div>
        <SearchBar
          onSearchChange={(query, isSearching) => setSearchQuery(query)}
          placeholder="Search models..."
          className="mb-4 mt-6 shadow rounded-xl "
        />
          ...
          {({ filteredItems }) => (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 w-full max-w-7xl mt-10">
              {filteredItems.map((item, index) => (
                <BottomCard key={index} item={item} index={index} icon={ArrayModels[index]} />
              ))}
            </div>
          )}
        </SearchController>
