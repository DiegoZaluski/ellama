...
const Control = () => {
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    if (deltaY !== 0) {
      ...
      newHeight = Math.max(
        ...
        Math.min(
          MAX_HEIGHT, 
          startSize.current.height + (deltaY * dragSensitivity.current)
        )
      );
    }
    ...
    if (deltaX !== 0) {
      newWidth = Math.max(
        ...
        Math.min(
          MAX_WIDTH, 
          startSize.current.width - (deltaX * dragSensitivity.current)
        )
      );
    }
  ...
  const calculateProportionalSizes = () => {
    ...
    // CONTROL BAR WIDTH
    const controlBarWidth = Math.max(
      200, 
      Math.min(440, size.width * 0.88) 
    );
    
    // BUTTON DIMENSIONS
    const buttonWidth = Math.max(
      40,
      Math.min(56, 56 * widthRatio) 
    );
    
    const buttonHeight = Math.max(
      32, 
      Math.min(48, 48 * widthRatio)
    );
    
    // BUTTON AND DECORATION SPACING
    const buttonMargin = Math.max(
      8, 
      Math.min(12, 12 * widthRatio)
    );
    
    // DECORATION CIRCLE SIZE
    const decorationSize = Math.max(
      20, 
      Math.min(32, 32 * widthRatio)
    );

    // CONTROL BAR HEIGHT
    const controlBarHeight = Math.max(
      56,
      Math.min(70, 70 * heightRatio)
    );
    ...
    // BALL ICON SIZE
    const ballIconSize = Math.max(
      20,
      Math.min(24, 24 * widthRatio)
    );

    // BUTTON TRANSLATE OFFSET
    const buttonTranslateX = Math.max(
      90,
      Math.min(112, 112 * widthRatio)
    );
