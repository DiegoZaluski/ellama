...
const ThinkButton = (deepSearch: ThinkButtonProps, className?: string ) => {
    ...
    return (
        <div className={`w-full h-full overflow-hidden rounded-full border border-black ${className} ${thinking? 'outline outline-1 outline-white': ''}`}>
            <button onClick={HandlerClick} type="button" className={`w-full h-full flex flex-row items-center justify-center p-2`}>
                <FaLightbulb size={12} />
